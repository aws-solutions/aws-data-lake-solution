...
function processRequest(event, ticket, cb) {
    ...
    if (event.resource === '/search') {
        ...
        _metadata.search(event.queryStringParameters.term, function(err, data) {
            if (err) {
                console.log(err);
                _response = buildOutput(500, err);
        ...
        _metadata.indexDocument(event.body, function(err, data) {
            if (err) {
                console.log(err);
                _response = buildOutput(500, err);
                return cb(_response, null);
        ...
        _metadata.deleteDocument(event, function(err, data) {
            if (err) {
                console.log(err);
                _response = buildOutput(500, err);
                _accessLog.logEvent(event.requestContext.requestId, servicename, ticket.userid, _operation,
